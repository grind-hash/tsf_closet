...
トークモード（romance 専用、手番を消費しない会話）: `POST /adventure/runs/{id}/talk/stream`（`AdventureTalkRequest.user_input`、上限 `ROMANCE_TALK_INPUT_MAX`）→ `stream_talk` が `romance_talk_system_prompt`（攻略対象として一人称のセリフだけ、`hidden_preferences` はほのめかしのみ、`_speech_rule_from_state` 併記）で `generate_feeling_stream` を流し、SSE `status{phase:talk}` / `talk_chunk` / `talk_done{user_entry, partner_entry, turn_count}` / `cost` / `complete` を返す。`turn_count`・`status`・`sim`・`AdventureTurn` には触れず、`state_json["talk_log"]`（`append_talk_entry`、上限 `ROMANCE_TALK_LOG_MAX`=40、ID はサーバ生成）だけを `_persist_locks` 下で書く。返答は `normalize_talk_reply` で `名前「」` を剥がす。次の手番は `_build_turn_contexts` が `recent_talk_entries(state, turn_count)`（最後の手番以降の分）を `turn_context["recent_talk"]` に載せ、`ROMANCE_RECENT_TALK_GUIDANCE`（narrative/resolution 共通。連続性には使うが時間は経過せず採点に影響させない）で扱いを定める。`talk_log` は `_lean_state_for_llm` で除外し `_REWIND_KEEP_KEYS` には入れない（巻き戻し先のスナップショットに従う）。`_serialize_run.talk_log`（romance のみ）で配信。FE は `AdventureContext.submitTalk` / `talking` / `talkDraft` / `pendingTalkInput`（`streaming || talking` で手番送信と相互排他）、入口は行動パネル見出しのセグメント型トグル「行動 / トーク」（既存 `.adventure-segments`、画面ローカル state、run 切替で行動へ戻る）。トーク中は選択肢・romance ボタン・選択肢再生成・数字キーを止め、`talk_log.filter(after_turn === turn_count)` を `.adventure-talk-thread` に話者名付きで出し、自由入力欄が `submitTalk` へ切り替わる。ログモーダルにはまだ載せない。

セリフ読み上げ（romance）: `hooks/useAdventureVoice.ts` が専用 `Audio` で AivisSpeech（`ensureAivisEngineRunning` → `synthesizeSpeech`、話者は `ttsStyleId ?? ttsSpeakerId`）を再生する。ChatContext の `playMessageAudio` は既定ミュートかつチャット専用バーに紐づくため使わない。設定は localStorage `adventure_voice_prefs`（`utils/voicePreferences.ts`、既定 OFF・音量 50%。音声再生の初期音量は BGM・通常ゲームの TTS も含め 100% でなく 50% にする）で、UI は♪ボタンのポップオーバー（`AdventureBgmControl`、aria-label は「サウンド設定」）に BGM と並べて置き、グローバル `ttsEnabled` が OFF のときは隠さず disabled + `adventure.voice.disabledHint`。トリガは (1) 新しい手番の到着（初回ロード・run 切替では読まない）で `utils/adventureDialogue.ts` の `partnerLines`（`名前「…」`/`名前：…` 行だけ。名前で始まる地の文は除外）を `joinForSpeech` して読む、(2) `talk_log` 末尾の攻略対象エントリが増えたときに `stripStageDirections` した返答を読む、(3) メッセージ窓メタ行の🔊で表示中フレーム/最新返答を再読み上げ。再生中は `useAdventureBgm(...).setDucked(true)` で BGM を `BGM_DUCK_FACTOR`(0.3) 倍にする。台本形式の本文は romance のとき `AdventureScriptText`（`parseDialogueSegments`）で話者ラベル付きに描く（名前付き行が無ければ従来どおり1段落）。3D モデル表示中（`avatarActive`）で読み上げ可能なら (0) 先読みとして、`narrative_done` で `AdventureContext.narrativeSettled` が真になった時点の `streamingNarrative` から同じ抽出で読み始め、判定・保存（`turn`）を待たない。声のキーは到着・先読みとも `turnVoiceKey` = `turn:{runId}:{turn_number}`（turn id は先読み時点で未確定）で、先読みした手番は `earlySpokenRef` に控えて (1) が読み直さない（控えはストリーム終了で消す。巻き戻し後の同番号でも読める）。本文ストリーム中の 🔊 も同じキーなので先読みの停止に使える。
