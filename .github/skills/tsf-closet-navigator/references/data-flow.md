...
1on1 立ち絵モード（romance 専用、`state_json["one_on_one_mode"]`、既定 OFF）: 攻略対象の立ち絵をステージ中央に1枚だけ置き、手番の画像生成を背景と攻略対象の立ち絵に限定する。`stream_turn.visual_producer` は `one_on_one` のとき `enable_composite=False` / `skip_player_portrait=True` を強制し、背景は `romance_location_key`（`adventure_romance.py`）で正規化した現在地が前手番から変わったときだけ `_ensure_romance_background_unlocked(slot=None)` を呼ぶ（キャッシュキーは時間帯を含まない現在地のみ。`strip_romance_time_of_day` で昼夜タグを落として生成する）。既存の `should_generate_image` ゲートは迂回しない（現在地の変化は `_visual_state_changed` を真にする）。開幕は主人公立ち絵も1枚作る（主人公ドックとモード解除後の参照に要る）が合成は作らず、開幕背景を現在地キーで `background_cache` に登録して手番1の描き直しを防ぐ。全 romance run の visual プロンプトに `ROMANCE_LOCATION_STABILITY_GUIDANCE`（場所が変わらない限り `visual_state.location` を前手番の文字列のまま写す）が載る。本文は `romance_script_format_guidance(partner, player)` の台本形式（`名前「セリフ」` の独立行）で、`_TurnContexts.script_names` が 1on1 のときだけ立ち、narrative/director プロンプト末尾に付く（`preview_turn_prompts` も同じ）。設定は `POST /runs` / `PATCH /runs/{id}/settings` の `one_on_one_mode`（romance 以外では無視）、`_serialize_run.one_on_one_mode`、`_REWIND_KEEP_KEYS` に含め `_lean_state_for_llm` で除外。↻は `AdventureImageRequest.target="partner"` → `generate_partner_portrait`（`_partner_portrait_tags_from_state` で state からタグを組み、最新ターンの `state_delta_json["partner_portrait_path"]` も差し替え）→ SSE `partner_image`。FE は `AdventureStageFrame.kind="partner"`（立ち絵の無い手番もフレームにし背景・立ち絵を引き継ぐ）、`portraitSource` は null、攻略対象スプライトに `adventure-stage__portrait--solo`、進捗は `partner` 工程だけ（`parsePhaseStep` が `partner` を通すよう修正済み。以前は捨てられ通常 romance でも進捗が主人公工程に張り付いていた）。トグルは setup の画像オプションと⚙ポップオーバーの両方（`adventure_setup_prefs.oneOnOneMode` に保存）で、ON 中は合成/主人公立ち絵トグルを隠さず `*OneOnOneHint` の文言で無効を説明する。
