...
セリフ読み上げ（romance）: `hooks/useAdventureVoice.ts` が専用 `Audio` で AivisSpeech（`ensureAivisEngineRunning` → `synthesizeSpeech`、話者は `ttsStyleId ?? ttsSpeakerId`）を再生する。ChatContext の `playMessageAudio` は既定ミュートかつチャット専用バーに紐づくため使わない。設定は localStorage `adventure_voice_prefs`（`utils/voicePreferences.ts`、既定 OFF）で、UI は♪ボタンのポップオーバー（`AdventureBgmControl`、aria-label は「サウンド設定」）に BGM と並べて置き、グローバル `ttsEnabled` が OFF のときは隠さず disabled + `adventure.voice.disabledHint`。トリガは (1) 新しい手番の到着（初回ロード・run 切替では読まない）で `utils/adventureDialogue.ts` の `partnerLines`（`名前「…」`/`名前：…` 行だけ。名前で始まる地の文は除外）を `joinForSpeech` して読む、(2) `talk_log` 末尾の攻略対象エントリが増えたときに `stripStageDirections` した返答を読む、(3) メッセージ窓メタ行の🔊で表示中フレーム/最新返答を再読み上げ。再生中は `useAdventureBgm(...).setDucked(true)` で BGM を `BGM_DUCK_FACTOR`(0.3) 倍にする。台本形式の本文は romance のとき `AdventureScriptText`（`parseDialogueSegments`）で話者ラベル付きに描く（名前付き行が無ければ従来どおり1段落）。
