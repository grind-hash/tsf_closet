...
> 最終検証: 2026-08-28 | 通常ゲームとAdventureは別のストリーム契約を持つ
>
...
preset="romance"（恋愛シミュレーション）は自動生成のみで、日数(5〜30日、1日=昼夜2ターン)を `scenario_max_turns=日数×2` として送る（ルーターの `scenario_max_turns` 上限は `SCENARIO_MAX_TURNS_REQUEST_MAX`=max(30, 30日×2)=60。非 romance の超過分は `clamp_generated_max_turns` が30に丸める）。好感度・金銭・ギフト採点・告白成否は `services/adventure_romance.py` の純関数が決定し、LLM は数値を書かない。告白ラインは `romance_confession_threshold(total_days)` で日数にスケールする（1手 `ROMANCE_CONFESSION_PACE` 想定、上限75。15日以上=75で飽和、7日=41、5日=32。日数不明の旧データは75）。会話ターンの affection_delta は resolution ガイダンスの採点基準（前向き+2、関心・ヒントに刺されば+3、平坦+1、+0は気まずい/反復のみ）に従い、±3クランプは従来どおり。現実改変で「交際を始める」を明示宣言した場合は resolution の `start_dating`（reality_alter ターン限定で有効）が告白成功と同じ扱い（全 milestone + success）になる。専用ボタン（バイト/ギフト/属性付与/告白）と重複する選択肢は narrative/resolution 両ガイダンスで禁止。day/slot は `turn_number` から導出し状態に保存しない。`state_json["sim"]` に相手・カタログ・隠し好み(`hidden_preferences`)を持ち、`_serialize_run` と `turn` イベントには隠し好みを除いた `public_sim_view` だけを `sim` として載せる。ターンは `input_kind: gift|work|confess`（gift は `gift_id` 併送、資金不足はターン未消費のエラー）を追加。属性付与は既存の現実改変機構を再利用し、HUD の現実改変チップを「付与した属性」表示に差し替える。開始素材で選んだ人物は主人公ではなく攻略対象（NPC として main_characters / npc_tags に描画、外見は `sim.partner_appearance`）。主人公（自分）はテンプレートキャラクター（`romance_player_character_id`、既定 char1）または特定セッション時点の変身状態（`romance_player_session_id`＋`romance_player_history_id`、指定時はテンプレより優先で `_build_snapshot` を流用）で、開始画像・外見ロック・立ち絵は主人公側へ差し替え、FE は選択を localStorage の adventure_setup_prefs に保存する。主人公の呼び名（攻略対象がセリフで呼ぶ名前）は `romance_player_name`（上限 `ROMANCE_PLAYER_NAME_MAX_LENGTH`=40、`normalize_player_name` で1行化）で上書きでき、空ならテンプレート名またはセッションの `character_name` へ倒して `sim.player_name` に入る。FE は選択したキャラクターの名前を既定値として欄に埋め、未編集なら選択変更に追従し、編集済みなら保持して adventure_setup_prefs に保存する。`_speech_style_instruction` は `sim.player_name` があると「セリフ内では主人公を名前で呼び、語りの『あなた』を名前として使わない」規則を SPEECH REGISTER に足す（二人称の語りがセリフへ漏れて「あなたさん」と呼ばれた不具合の対策。`romance_setup_system_prompt` の呼びかけ方も player_name から組み立てさせる）。リプレイで主人公選択を元 run から引き継ぐときは呼び名も `_romance_replay_player_name` で復元する。テンプレキャラの外見ロックは `_romance_template_player_appearance` が characters.json の gender から性別トークン(male, 1boy / female, 1girl)を明示する（base_tags に無いと画像が女性寄りに描画されるため）。director/narrative/visual の同一性署名の複写指示は sex を含み、`ROMANCE_VISUAL_GUIDANCE` は player_tags への性別トークン復唱を要求する。romance 時は HUD・メッセージウィンドウ・行動ボタンを `adventure-hud--romance` / `adventure-messagebox--romance` のローズトーンに切り替える。非合成モードでは攻略対象の立ち絵を主人公と並置表示する: 参照は run ディレクトリの `partner_initial.*`。最新パスは `state_json["partner_portrait_path"]`（開幕分は `opening_partner_portrait_path`）、ターン別パスは各ターンの state_delta に保持し（いずれも `_lean_state_for_llm` で除外）、開幕に加えて主人公と同様に毎ターン `_generate_partner_portrait_unlocked` で直列生成する（step "partner" の status を送出、FE 進捗バーにも partner 工程あり。失敗はターンを止めず前の1枚を維持）。ただしターン中のタグ決定は `_romance_partner_turn_portrait_tags` が行い、相手がその手番の `main_characters` に居なければ空文字を返して**描き直さず前の1枚を残す**。居ない相手を `sim.partner_appearance` から描くと、服装情報が無いぶん裸で描かれ、かつ改変前の古い外見へ戻って見える（実際に発生した不具合）。`npc_tags` を取れないが相手は場面に居る場合だけ `sim.partner_appearance` ＋ エントリの clothing で補う。生成ヘルパは state を自前でDB保存するが、stream 終端の全 state コミットに上書きされるため、consumer が受けたパスを必ず next_state へ反映してからコミットする。配信は SSE `partner_image`、`_serialize_run.partner_portrait_url` / `opening_partner_portrait_url`、`_serialize_turn.partner_portrait_url` で、FE は過去フレーム閲覧中もその手番時点の相手立ち絵をステージ・モーダル（攻略対象チップ/シーン並置）に表示する（欠けたターンは直前の1枚を引き継ぐ）。`_serialize_turn` は state_delta_json に sim があるターン（=romance）へ、確定時点の `sim`（public_sim_view）と攻略対象の様子 `partner_note`（main_characters の該当エントリ description）を載せ、`_serialize_run` は開幕フレーム(手番0)用に `opening_sim`（開始値は定数のため `opening_sim_view` が現在の sim から再構成）も返す。`public_sim_view` は現実改変で書き換わる `partner_appearance` も配信し、FE は主人公ドック内の第2セクションに攻略対象の外見（`sim.partner_appearance`）と服装（`visual_state.main_characters` を `partner_name` の部分一致で引く）を主人公と同じ体裁で並べる。`partner_appearance` はターンで変化するため `opening_sim` の同項目は開幕時の値に戻せない（現在値が入る）。開幕フレームの表示に使わないこと。ターン詳細モーダルはこれを使い攻略対象カード（名前・好感度・段階・様子）と Day 表記（day/slot は FE の `romanceDaySlot` で turn_number から再導出）を表示し、`ImagePreviewModal` の `className` 経由の `adventure-preview--romance` でローズテーマ化する。リプレイ（もう一度遊ぶ）は「同一シナリオ・新規 sim」として対応: `create_run` の replay 分岐が素材（`source_session_id`/`source_history_id`＝攻略対象）と主人公選択（`sim.player_character_id`、`session:` 形式なら `sim.player_history_id` 併用。リクエストに `romance_player_*` があればそちら優先）を元 run から引き継ぎ、日数は `clamp_romance_max_turns` で丸める。相手プロフィール・ギフトカタログ・隠し好みは毎回 LLM で再生成される（意図した仕様）。攻略対象の口調 `sim["partner_speech_style"]` も同じ扱いで、`RomanceSetupOutput.partner_speech_style` として `romance_setup_system_prompt` が人物像から生成し（`partner_profile` からは「話し方」の記述を外して役割の重複を消した）、ユーザーがセットアップで書いた上書き値があればそちらが優先される。`ROMANCE_NARRATIVE_GUIDANCE` が「毎手番この口調を守り、主人公の口調へ収束させない。空なら partner_profile から導く」と定め、空欄でも従来どおり動く。ミッション案の自動生成プロンプト（`_setup_system_prompt` の romance 分岐）は、恋敵を扱う機構が実装されていないため complications の例示から `rivals` を外してある。

対面会話モード（romance 専用、`state_json["companion_mode"]`、既定 OFF。旧称 1on1）: Grok の Ani のように、目の前に立つ攻略対象と **1手番＝1往復** で会話する。物語は半日枠でなく **3ビート**「(1) 相手の見た目の反応の地の文1行 → (2) 攻略対象のセリフ `名前「…」` 1〜2行 → (3) 場面が求めるとき相手側からの展開（問い返し・小さな行動・話題の一歩）」で全体 2〜5 文（`romance_companion_narrative_guidance(partner, player)` が `_ROMANCE_NARRATIVE_CORE` に半日ペーシングの代わりに付く）。拒絶・怒り・驚愕・沈黙など**短さ自体が答えになる場面は短いまま**にし、問い返しや雑談で埋めない（"never pad"）。末尾の `romance_script_format_guidance` は行数を「ビート規則に従う（通常1〜2行、行数合わせで足さない）」と定め、以前の「少なくとも2回話せ」との矛盾（後に付く台本指示が勝ち、返答が1行に潰れる原因）を解消済み。判定側は `ROMANCE_COMPANION_RESOLUTION_GUIDANCE` で選択肢を「次に言う/する一言」にし、相手の最後の行が質問なら少なくとも1つはそれに答える選択肢にする。speech/narration の分離は台本形式そのもの（`名前「…」` 行＝セリフ、他＝地の文）で、FE の `partnerLines` が TTS 用にセリフ行だけを読む。JSON 化やバックエンド抽出は行わない（`narrative_chunk` のストリーム表示と先読み読み上げが壊れる）。トークモードはセリフのみの設計なので対象外。**昼・夜の概念は使わない**: `_build_turn_contexts` は `turn_context["romance_resolution"]` から `day/slot/next_day/next_slot`（`_COMPANION_HIDDEN_RESOLUTION_KEYS`）を落とし `companion_mode: true` を載せ、`regenerate_choices` は `romance_next_slot` を出さず、開幕プロンプトは `opening_slot`/`total_days` の代わりに `total_turns`/`companion_mode` を渡す。内部の `sim.total_days = max_turns // 2` と day/slot 導出はそのまま（告白ラインのスケールに使う）だが UI・プロンプトには出さない。セットアップは日数でなく **ターン数**（FE `COMPANION_TURN_OPTIONS` 10/14/20/30/40/60、`scenario_max_turns` にそのまま送る）で、`POST /setup/generate` に `companion_mode` を送ると `_setup_system_prompt` / `romance_setup_system_prompt(companion=True, turns=...)` がゴール文面をターン数で書く。FE の設定トグルはセットアップの romance 生成ブロック（日数/ターン数セレクトの直上）と⚙ポップオーバーの2箇所。HUD の Day タイルは `adventure.companion.*` で「ターン n / N」に差し替え、ライトボックスの日数表記も出さない。画像は背景と攻略対象の立ち絵に限定する。`stream_turn.visual_producer` は `companion` のとき `enable_composite=False` / `skip_player_portrait=True` を強制し、背景は `romance_location_key`（`adventure_romance.py`）で正規化した現在地が前手番から変わったときだけ `_ensure_romance_background_unlocked(slot=None)` を呼ぶ（キャッシュキーは時間帯を含まない現在地のみ。`strip_romance_time_of_day` で昼夜タグを落として生成する）。既存の `should_generate_image` ゲートは迂回しない（現在地の変化は `_visual_state_changed` を真にする）。開幕は主人公立ち絵も1枚作る（主人公ドックとモード解除後の参照に要る）が合成は作らず、開幕背景を現在地キーで `background_cache` に登録して手番1の描き直しを防ぐ。全 romance run の visual プロンプトに `ROMANCE_LOCATION_STABILITY_GUIDANCE`（場所が変わらない限り `visual_state.location` を前手番の文字列のまま写す）が載る。本文は `romance_script_format_guidance(partner, player)` の台本形式（`名前「セリフ」` の独立行）で、`_TurnContexts.script_names` が 対面会話モードのときだけ立ち、narrative/director プロンプト末尾に付く（`preview_turn_prompts` も同じ）。設定は `POST /runs` / `PATCH /runs/{id}/settings` の `companion_mode`（romance 以外では無視）、`_serialize_run.companion_mode`、`_REWIND_KEEP_KEYS` に含め `_lean_state_for_llm` で除外。↻は `AdventureImageRequest.target="partner"` → `generate_partner_portrait`（`_partner_portrait_tags_from_state` で state からタグを組み、最新ターンの `state_delta_json["partner_portrait_path"]` も差し替え）→ SSE `partner_image`。FE は `AdventureStageFrame.kind="partner"`（立ち絵の無い手番もフレームにし背景・立ち絵を引き継ぐ）、`portraitSource` は null、攻略対象スプライトに `adventure-stage__portrait--solo`、進捗は `partner` 工程だけ（`parsePhaseStep` が `partner` を通すよう修正済み。以前は捨てられ通常 romance でも進捗が主人公工程に張り付いていた）。トグルは setup の画像オプションと⚙ポップオーバーの両方（`adventure_setup_prefs.companionMode` に保存）で、ON 中は合成/主人公立ち絵トグルを隠さず `*CompanionHint` の文言で無効を説明する。攻略対象の立ち絵を**描かなかった手番はその理由を記録する**: `state_json["partner_portrait_status"]`（語彙は `consts/adventure_partner_portrait.py` が唯一の情報源。`generated` / `not_requested`（毎ターン描く OFF・3D モデル表示中）/ `scene_unchanged`（`should_generate_image` が偽）/ `partner_absent`（`main_characters` に相手が居ない）/ `failed` / `visual_failed`）。`stream_turn.visual_producer` の各ゲートがローカル helper `skip_partner` で `("partner_skipped", code)` を queue へ積む（消費ループは `resolution_done and visual_done and portrait_done and image_done` で読み取りを止めるため、**終了判定フラグを立てるメッセージより前に積む**こと）。romance run はコミットで毎手番必ず上書きする（state_delta は全 state なので、書かないと前手番の値が残る。None は「記録なし」）。`_serialize_turn` / `_serialize_run` が `partner_portrait_status` を配信し、`_lean_state_for_llm` で除外、`_REWIND_KEEP_KEYS` には入れない。↻（`generate_partner_portrait` / `_generate_partner_portrait_unlocked`）は `generated` に戻す。FE は `AdventureStageFrame.partnerStatus` / `partnerInherited`（status があればそれを信じ、無い旧ターンだけ前フレームと URL 一致で判定。SSE 由来の turn は URL に API_BASE が付かないため URL 比較を主にしない）を立て、立ち絵をステージに出しているとき（非合成または対面会話、3D モデル非表示、ストリーム中でない）だけメッセージ窓メタ行の `adventure-messagebox__portrait-note` とライトボックスの攻略対象カードに `adventure.partnerPortrait.note`（理由付き）を出す。↻ 後は `AdventureContext` の `partner_image` ハンドラが該当 turn の status も `generated` に書き換える（再取得しないため）。あわせて `last_image_prompt` の書き戻し条件を「主人公**または攻略対象**の立ち絵を描いた手番」に広げた: 対面会話モードは主人公立ち絵を常に省くため、従来は run 全体で開幕値のまま凍り、「場面に変化なし」判定・visual LLM の `previous_image_tags`・↻ の `_partner_portrait_tags_from_state` がすべて開幕タグ基準になっていた。

対面会話モードの 3D モデル（VRM、既定「なし」）: 設定画面「3Dモデル (VRM)」で登録した VRM（`POST /api/avatars` multipart → `avatar_service.save_upload` が GLB/VRM を検証し `data/avatar_models/{id}.vrm` に保存、`AvatarModel` 行）を、run の `state_json["companion_avatar_id"]` に割り当てると（`POST /runs` / `PATCH /runs/{id}/settings` の `companion_avatar_id`。`"none"` で解除、None は据え置き、未登録 id は `avatar_not_found`(404)。`_REWIND_KEEP_KEYS` に含め `_lean_state_for_llm` で除外。`_serialize_run` が `companion_avatar_id` / `companion_avatar_url`(`/avatars/{id}/file`、DB を引かない) を配信）、FE は攻略対象の立ち絵 `<img>` の代わりに `components/adventure/avatar/CompanionAvatarStage.tsx`（`React.lazy`、three.js は別チャンク）を `.adventure-stage__frame` に置く（背景とスクリムはそのまま）。選択 UI はセットアップの対面会話トグル直下と⚙ポップオーバーの `adventure-setup-avatar` セレクト（対面会話 OFF でも隠さず文言で説明、登録 0 件なら設定画面へのリンク、削除済み id は disabled option）で、セットアップの選択は `adventure_setup_prefs.companionAvatarId` に保存し登録済みのときだけ送る。表情・身振りは新しい LLM 呼び出しを足さず既存の呼び出しに相乗りする: 手番は `AdventureRomanceResolutionOutput` / `AdventureDirectorOutput` の `partner_expression` / `partner_gesture`（語彙は `consts/companion_avatar.py` が唯一の情報源。寛容 validator で語彙外は None。`_resolution_system_prompt(romance, companion=True)` のときだけスキーマと `avatar_resolution_instruction()` を載せる。`stream_turn` の詰め替え→`_merge_output` が毎手番上書き→`_serialize_turn` の romance ブロックで配信）、キャラチャット（adventure 種）では `adventure_persona_prompt(header_instruction=avatar_talk_header_instruction())` が先頭ヘッダ行 `[expression=<key> gesture=<key>]` を要求し、`character_chat_service._HeaderBuffer` が改行か 64 文字までバッファして `parse_talk_header` で剥がしてから `chat_chunk` を流し、メッセージ meta の `expression` / `gesture` として配信する。FE は `AdventureStageFrame.partnerExpression/partnerGesture`（`normalizeAvatarExpression` で正規化）から表情・身振りを導き、身振りの再生キーは読み上げ可能なら「声の開始時の `voice.currentKey`」（到着と🔊再読み上げの両方で発火）、不可なら `voiceReplayKey`。口パクは `useAdventureVoice.getLevel()`（`utils/voiceLevelMeter.ts`、AivisSpeech は `audio_query` のモーラ長がダミー 0.0 のため音量解析方式）。アバター表示中は `performSubmitTurn` が `generate_partner_portrait:false` を送り（開幕立ち絵はフォールバック用に残る）、進捗工程表示と見積もりも `drawPartnerEveryTurn && !avatarActive` で揃える。読込失敗（削除済みモデルの 404、WebGL 不可、Context Lost）は `onError` → `AdventureContext.companionAvatarFailed` で立ち絵へ戻し warning 通知を出す。VRM 0.x は `VRMUtils.rotateVRM0` で +Z 向きに揃え、`surprised` が無いモデルは neutral 扱い。3D モデル表示中は手番の進捗でステージを覆わない（`quietStage` = `avatarActive`。読み上げ OFF でも適用）: `.adventure-stage__loading` と背景の減光（`is-generating`）を出さず、本文ストリーム中はメッセージ窓のカーソル（`narrativeSettled` で消える）だけ、`narrative_done` 以降は行動パネルの `.adventure-controls__progress`（`controlsProgressVisible`。選択肢再生成・画像再生成の進捗もここ）にスピナー＋phase 文言を出し、その間は再生成ボタンの「選択肢を生成中...」への差し替えをしない。身振りの再生キーは、声のキーが `turn:` 系なら表示中フレームの `turnVoiceKey` と一致したときだけ通す（先読み中はフレームがまだ前手番なので前手番の身振りを再生せず、`turn` 到着でフレームが増えた時点で新しい身振りを再生。表情は `selectedFrame.partnerExpression` で同時に切替）。対象外: FBX/PMX、VRMA/Mixamo 素材、モーラ長リップシンク、非対面会話ビュー、現実改変による外見変化のモデル反映。

3D モデルの衣装差分（着替え）: 設定画面で同じ `character_name` に分類した VRM（登録時はファイル名 `名前_衣装_髪型Ver.vrm` から `classify_avatar_filename` が自動で入れ、`PATCH /api/avatars/{id}` で付け替える。run は個々のモデル ID を参照するため分類変更で壊れない）が 2 件以上あるとき、`stream_turn` と `preview_turn_prompts` が `_companion_outfit_options`（`list_avatar_variants` を差分ラベル順に `{key:"1"…, id, label, current}` へ。対面会話モードでモデル表示中だけ。取得失敗は空で手番を止めない）を `_build_turn_contexts(outfit_options=)` へ渡し、`turn_context["partner_wardrobe"]`（`_wardrobe_context`: `{current:{key,label}, options:[{key,label}]}`。ID は見せない）として物語と判定の両方に載せる。物語プロンプトは `wardrobe=True` で `avatar_wardrobe_narrative_instruction()`（player_input が求めるか場面が必要とするときだけ着替え、ラベルで分かるよう描写。台本形式の指示より前に置く）、判定は `outfit_keys` で `"partner_outfit":"1|2"` と `avatar_wardrobe_resolution_instruction()` を載せ、`AdventureResolutionOutput.partner_outfit`（`normalize_avatar_outfit_key` の寛容 validator）を `_resolve_outfit_choice`（キー→ID→ラベルの順で照合、候補外は None＝据え置き）で登録 ID に写し、変わっていれば `_merge_output` の前に `state["companion_avatar_id"]` を更新する（state_override 経由で state_delta にも載る）。`_serialize_turn` の romance ブロックが `companion_avatar_id` / `companion_avatar_url` を配信し、FE は `AdventureContext` の `turn` イベントで id が変わったときだけ `activeRun.companion_avatar_id/url` を差し替え（`avatarModelFileUrl`）、`CompanionAvatarStage` が `fileUrl` の変化で再読込する（`companionAvatarFailed` も id 変化でリセット）。選択肢の再生成（`_generate_fresh_choices`）とトーク（「場面は進まない」ルールのまま）では着替えない。巻き戻しは `_REWIND_KEEP_KEYS` により現在のモデルを保つ（次の手番の判定が本文に合わせ直す）。選択 UI は `AdventureScreen` の `AvatarModelOptions`（キャラクターごとの `optgroup`、差分ラベル表示。未分類はモデル名）と `AvatarWardrobeHint`（選択中モデルに差分が 2 件以上あるときだけ `adventure.avatar.wardrobeHint`）で、セットアップと⚙ポップオーバーの両方に置く。

トーク（romance 専用、手番を消費しない攻略対象との会話）は Adventure の中には無く、行動パネルの「トーク」→ `POST /character-chat/threads/adventure/{run_id}` → `/talk/:threadId` でキャラチャット（adventure 種）へ移動して行う。次の手番は `_recent_chat_for_run` が `character_chat_service.recent_adventure_messages(run_id, after_turn=turn_count)` で「前の手番以降の発言」を読み、`_build_turn_contexts(recent_talk=)` → `turn_context["recent_talk"]` と `ROMANCE_RECENT_TALK_GUIDANCE`（連続性には使うが時間は経過せず採点に影響させない）で扱う。旧 `state_json["talk_log"]` はスレッド作成時に `consume_talk_log` で取り込んで消す。

セリフ読み上げ（romance）: `hooks/useAdventureVoice.ts` が専用 `Audio` で AivisSpeech（`ensureAivisEngineRunning` → `synthesizeSpeech`、話者は `ttsStyleId ?? ttsSpeakerId`）を再生する。ChatContext の `playMessageAudio` は既定ミュートかつチャット専用バーに紐づくため使わない。設定は localStorage `adventure_voice_prefs`（`utils/voicePreferences.ts`、既定 OFF・音量 50%。音声再生の初期音量は BGM・通常ゲームの TTS も含め 100% でなく 50% にする）で、UI は♪ボタンのポップオーバー（`AdventureBgmControl`、aria-label は「サウンド設定」）に BGM と並べて置き、グローバル `ttsEnabled` が OFF のときは隠さず disabled + `adventure.voice.disabledHint`。トリガは (1) 新しい手番の到着（初回ロード・run 切替では読まない）で `utils/adventureDialogue.ts` の `partnerLines`（`名前「…」`/`名前：…` 行だけ。名前で始まる地の文は除外）を `joinForSpeech` して読む、(2) メッセージ窓メタ行の🔊で表示中フレーム/最新返答を再読み上げ。再生中は `useAdventureBgm(...).setDucked(true)` で BGM を `BGM_DUCK_FACTOR`(0.3) 倍にする。台本形式の本文は romance のとき `AdventureScriptText`（`parseDialogueSegments`）で話者ラベル付きに描く（名前付き行が無ければ従来どおり1段落）。3D モデル表示中（`avatarActive`）で読み上げ可能なら (0) 先読みとして、`narrative_done` で `AdventureContext.narrativeSettled` が真になった時点の `streamingNarrative` から同じ抽出で読み始め、判定・保存（`turn`）を待たない。声のキーは到着・先読みとも `turnVoiceKey` = `turn:{runId}:{turn_number}`（turn id は先読み時点で未確定）で、先読みした手番は `earlySpokenRef` に控えて (1) が読み直さない（控えはストリーム終了で消す。巻き戻し後の同番号でも読める）。本文ストリーム中の 🔊 も同じキーなので先読みの停止に使える。
...
TSF シナリオを経由しないキャラクターとの 1 対 1 の会話（Adventure のトークもここへ統合）。`/talk`（Hub）と `/talk/:threadId`（Room）、Provider は `/talk` 配下だけ、ゲートは `experimentalCharacterChatEnabled`（既定 OFF）。

```text
CharacterChatHub / CharacterChatRoom
  ↓ CharacterChatContext
apis/characterChat.ts
  ├─ GET /character-chat/threads、POST /threads/base（案内役キャラ。冪等）、POST /threads（開始素材から作成）
...
3 種類のキャラクター。**adventure 種**（`kind="adventure"`、`source_run_id`。恋愛シミュレーションの攻略対象。`_load_adventure_view` で run を毎回ライブで読み、好感度・段階・日数・ギフト・節目・現実改変ルール・直近の場面・所持品を `adventure_persona_prompt` の context に渡す。姿の既定は表示モードで決める: 合成モードなら相手が写る最新の場面画像（scene）、非合成/対面会話なら最新の攻略対象立ち絵（standing）。追従モードは `appearance_json.source.adventure_mode`（default / partner_portrait / scene）で、チャット側の着替え・描き直し・別素材は `custom` になり追従を止める（「シナリオの姿に合わせる」= `POST /appearance/adventure {mode: default}` で戻す）。run に 3D モデルがあれば返答に表情・身振りヘッダを要求し、FE は `CompanionAvatarStage` を立ち絵の枠に置く。run 削除後は persona の控えで会話だけ続く）、**案内役キャラ「セレナ」**（`kind="base"`。ユーザーごとに 1 スレッド。人物設定は `consts/character_chat.BASE_CHARACTER_PERSONA` + `APP_OVERVIEW` でメタ会話可。同梱立ち絵は `backend/images/character_chat/serena.png` で、無ければ `portrait_missing` を返し FE が配置パスの案内と「立ち絵を生成」（`portrait/stream`、txt2img）を出す）と、**セッション由来キャラ**（`kind="session"`。`build_source_snapshot` の画像・外見タグ・経緯・属性・統計と `resolve_session_identity` の名前・一人称、`play_memory_service.build_context` を **作成時点でスナップショット**して `persona_json` に持つ。元セッションが進んでも追従しない）。`User.memory_text` だけは毎回ライブで読み、`memory_block` の弱い枠（人格・口調はキャラ設定優先）で system prompt に載せる。 姿のタグは `_adventure_identity` が Adventure の攻略対象立ち絵と同じ `state["last_image_prompt"]["npc_tags"]` から組む（`_split_portrait_tags` で服装 / 同一性に分け、場面・構図タグは落とす）。`sim["partner_appearance"]` は開始素材の記述由来で行為タグが残り得るため、npc_tags が無い run の保険にだけ `identity_tags_only` を通して使う。
...
apis/promptExpander.ts
  ...
  ├─ /sessions, /sessions/{id}, /sessions/{id}/uploads, /entries, /images/{entry_id}, /entries/{entry_id}/mask
...
- 精密参照（V4.5 系のみ）: 設定 `use_precise_reference` / `reference_type` / `reference_strength` / `reference_fidelity`（既定 character / 0.85 / 1.0、Adventure の立ち絵と同じ）は `PromptExpanderSettings` JSON に、参照画像は Context の `reference`（i2i 元と独立、セッション内のみ）に持つ。FE は `referenceActive`（ON かつ V4.5 かつ画像あり）のときだけ `POST /generate` に `reference_kind`（history/entry/upload）＋ ID/base64 ＋種別/強度/忠実度を載せ、BE は `resolve_source` で bytes にして `character_references` に渡す。`reference_kind != none` が「参照を送った」唯一の根拠で、V5 では `precise_reference_requires_v45`(422)。エントリに参照元と強度を記録し、「このプロンプトで再生成」は history/entry 参照を再送する（upload 参照は再現不可）。参照付き生成は 1 枚 5 Anlas（設定応答 `anlas_per_reference`）を消費するため、`generate()` が共通 UI `AnlasConfirmDialog` の確認を挟む（抑止は sessionStorage `prompt_expander_anlas_warn_suppressed`）。
- インペイント（部分修正）: 設定 `use_inpaint`（既定OFF）。ベース画像は i2i 元（`source`）と共用し、マスクだけを Context の `inpaintMask` に持つ。FE は `inpaintActive`（ON かつ i2i 元あり かつマスクあり）のときだけ `POST /generate` に `inpaint_mask`（新規描画の data URL）または `inpaint_mask_entry_id`（保存済みマスクの再利用）を載せる。BE は bytes 化して `image_service.generate_image(mask_bytes=...)` へ渡し、既存の NovelAI 経路が `action=infill` とインペイント用モデルへ切り替える（4 モデルすべて対応）。強度・ノイズは i2i の値がそのまま `inpaintImg2ImgStrength` / noise になるので、UI に別スライダーを置かない。マスクは `{entry_id}_mask.png` として保存し、「このプロンプトで再生成」「欄へ復元」で同じ領域を使い回せる（立ち絵の表情差分を連続で作るため）。i2i 元が無い状態のマスクは `inpaint_requires_source`(422) で拒否し、黙って通常の i2i に落とさない。マスク編集 UI（`PromptExpanderInpaintModal`）は `react-sketch-canvas` で描き、プリセットは通常ゲームと同じ `/api/game/masks` を共用する。書き出しグリッドはベース画像の 1/8 で求める（`image_generation.py` の正規化側も同じ 1/8 に直した。104x152 固定のままだと landscape / square でマスクの縦横比が崩れる）。
- 背景透過: 設定 `transparent_background`（漫画モード中は無効。スイッチは隠さず文言で説明）。BE は送信用プロンプトにだけ V5=`transparent background, no shadow`（ネイティブ透過 PNG）／V4.5=`simple background, white background, no shadow` を足し、negative に複数ビュー抑止語（複数人を禁じる語は入れない）を冪等併合する。V4.5 は白背景の指定がモデルに無視されて背景が描かれることがあるため、設定 `transparent_emphasis`(0〜3、既定2) で背景タグ（`simple background` / `white background`）にだけ NovelAI の `{}` 強調（1 段 1.05 倍）を掛ける。`no shadow` は素のまま、V5 は常に無強調（ネイティブ透過なので不要）。強調記法が付くと素のタグとの重複判定が崩れるため、`merge_tags` は `clothing_layers.normalize_tag_for_match` で `{}` や `N.N::tag::` を外してから比較する。透過タグは `final_prompt` に残らず送信プロンプトにしか現れないので、FE は欄の直下に「送信時に追加」プレビューを出す（`constants/promptExpander.ts` の `transparentBackgroundTags` / `appendedTags` が backend のタグ表と `merge_tags` のミラー。片方だけ直さない）。`regenerateEntry` はエントリに強調段数を保存していないため現在の設定値を送る（送らないとルーター既定の 2 になる）。エントリには印 `transparent_background` だけを残し `final_prompt` は接尾辞なし（復元→生成で二重付与しない）。`/expand` にも同フラグを送り、system prompt に背景を書かせない規則を足す。FE は Adventure と同じ `imageAlpha.ts`（`useTransparentImage`、threshold 12）で表示時に切り抜き（V5 の透過画像は `hasMeaningfulAlpha` で素通し）、サーバー保存 PNG は V4.5 では白背景のままなので「透過PNGを保存」で切り抜き後の blob をダウンロードさせる。キャッシュ（`CACHE_LIMIT` 48）の退避は blob URL を revoke するが、revoke 済み URL は `<img>` に残ったまま「名前を付けて画像を保存」の再取得だけが失敗する（リロードで直るように見える）ため、表示中の URL は `useTransparentImage` が `retainTransparentImage` で保持し、保持中は退避しない。`onError` の原本フォールバックは保険。
