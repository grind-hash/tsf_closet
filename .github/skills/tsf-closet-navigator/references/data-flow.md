...
- 背景透過: 設定 `transparent_background`（漫画モード中は無効。スイッチは隠さず文言で説明）。BE は送信用プロンプトにだけ V5=`transparent background, no shadow`（ネイティブ透過 PNG）／V4.5=`simple background, white background, no shadow` を足し、negative に複数ビュー抑止語（複数人を禁じる語は入れない）を冪等併合する。エントリには印 `transparent_background` だけを残し `final_prompt` は接尾辞なし（復元→生成で二重付与しない）。`/expand` にも同フラグを送り、system prompt に背景を書かせない規則を足す。FE は Adventure と同じ `imageAlpha.ts`（`useTransparentImage`、threshold 12）で表示時に切り抜き（V5 の透過画像は `hasMeaningfulAlpha` で素通し）、サーバー保存 PNG は V4.5 では白背景のままなので「透過PNGを保存」で切り抜き後の blob をダウンロードさせる。一覧に透過エントリが並ぶため `CACHE_LIMIT` を 48 に上げ、退避で revoke された URL は `onError` で原本へ戻す。
