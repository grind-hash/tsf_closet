...
components/
  ...
  settings/
    ...
    CharacterChatRealWorldSettings.tsx キャラチャットのセレナの Web 検索・天気のトグル(localStorage)。`GET /api/settings/user` の `web_search_configured` / `weather_configured` が false なら説明文を添える(トグルは常に操作可能)。Web 検索を ON にするときだけ、Tavily の Acceptable Use Policy に従うこと(ポルノ・未成年者に関する不適切な内容・違法行為などの検索禁止)と、結果に不正確・不適切な内容が含まれ得ることへの同意を `ConfirmDialog` で求め、キャンセルなら OFF のまま
```
