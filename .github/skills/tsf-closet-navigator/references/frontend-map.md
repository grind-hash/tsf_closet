...
components/
  ...
  characterChat/
    ...
    CharacterChatProposalChip.tsx 案内役の部屋だけ、入力欄の上の段（`CharacterChatInput` の `toolbar`）に常設する「おすすめのプレイを聞く」。定型文を `request_play_proposal: true` で送る（送信中は無効）
    CharacterChatPlayProposalCard.tsx 返答の `meta.play_proposal` の提案カード（テーマ・理由・キャラクター・最初の指示を全文表示）。メッセージ窓（最新の返答。表示中は Room が `character-chat-room--with-proposal` で窓を高くする）とログドロワーに出し、「このキャラクターでプレイを始める」で `startProposedPlay` → `/play/{id}`。返答の処理中と、自分自身モードで自プロフィールが無いときは無効
    playProposal.ts             `toPlayProposal(meta)`: `kind: "play"` 以外や形の崩れた提案を null にする（後の版の `adventure` 等にはカードを出さない）
    ...
    CharacterChatThread.tsx     メッセージ一覧（ログドロワー内。仮吹き出し・下書きのキャレット・phase 別のスピナー付き進捗・調べたことのメタ行（`CharacterChatCitations`。Web 検索は送った検索語と出典リンク。利用規約で見送ったものは「見送り」と送らなかった検索語）・「姿が変わりました」バッジ・🔊 再読み上げ）
  ...
  settings/
    ...
    CharacterChatRealWorldSettings.tsx キャラチャットのセレナの Web 検索・天気のトグル(localStorage)。`GET /api/settings/user` の `web_search_configured` / `weather_configured` が false なら説明文を添える(トグルは常に操作可能)。Web 検索を ON にするときだけ、利用条件(i18n `settings.characterChatWebSearchTerms`。BYOK 前提の 5 行の要約・Tavily Acceptable Use Policy へのリンク・「詳細な注意事項」の折りたたみ(Tavily の規約の遵守・セーフガード・送信される情報・API キーと料金・検索結果・利用の制限の 6 節)・同意の一文)への同意を `ConfirmDialog` で求め、キャンセルなら OFF のまま。「Web検索の利用条件を読む」で同じ本文を閉じるボタンだけのダイアログ(`cancelLabel` 省略)で読み返せる
```
