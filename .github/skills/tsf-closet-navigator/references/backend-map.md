...
| `character_chat.py` | キャラチャットの作成 / 姿の差し替え（`CharacterChatSourceRequest`: セッション or Prompt Expander エントリ必須）と発言（`CharacterChatMessageRequest`、上限 `consts/character_chat.MESSAGE_MAX`。`use_web_search` / `use_weather` は設定画面のトグルで、案内役キャラだけに効く） |
...
| `/api/character-chat` | `routes/character_chat_router.py` | キャラチャット（TSF シナリオを経由しないキャラクターとの会話）。`GET /threads`、`POST /threads/base`（案内役キャラ「セレナ」のスレッド。冪等）、`POST /threads/adventure/{run_id}`（TSF シナリオ(恋愛シミュレーション)の攻略対象。run ごとに 1 件・冪等。初回に旧 `talk_log` を取り込む）、`POST /threads`（過去セッション / 履歴 / Prompt Expander エントリの人物から作成。201）、`GET/DELETE /threads/{id}`、`POST /threads/{id}/messages/stream`（SSE: `status{phase: plan|search|reply|portrait|memory}` / `chat_chunk` / `chat_done` / `portrait_image` / `portrait_error` / `cost` / `complete` / `error`）、`PUT /threads/{id}/appearance`（姿の差し替え）、`POST /threads/{id}/appearance/reset`（作成時点の姿へ戻す。adventure 種は「シナリオの姿に合わせる」）、`POST /threads/{id}/appearance/adventure`（adventure 種: `mode` default / partner_portrait / scene）、`POST /threads/{id}/portrait/stream`（立ち絵の描き直し。SSE。任意の JSON `{reference: current|scene|partner, use_precise_reference}`）、`GET /images/{thread_id}/{filename}`。エラーは `detail.code`（`thread_not_found` / `source_not_found` / `image_not_found` は 404、他 400） |
...
| `character_chat_prompts.py` | 判定 LLM / 返答本文 / 着替えの外見タグ更新 / スレッド要約のプロンプト（純関数）。判定 LLM に現実世界の調べ物を見せるのは `planner_system_prompt(real_world_kinds=)` で許可したときだけ（無ければ従来の文面）。案内役キャラの返答には `current_time_block`（いまの日時。オフセットは `%z`）と `real_world_block`（アプリの外の文章として扱わせる枠）を過去プレイの結果の後ろに置く。Web 検索を利用規約で見送った手番は `web_search_refusal_block` で、話し始めの一文で謝って理由を伝えてから自分の知識で答えさせる。ユーザーメモリは `memory_block` の弱い枠で包む（`build_memory_priority_instruction` の「最優先」は使わない）。着替えの外見タグは英語限定: 出力に日本語(CJK)が混じれば `non_english_tag_parts` で検出し、混じった要素を `rejected_previous_output` として 1 回だけ再生成、それでも残れば `appearance_tags_not_english` を投げて立ち絵は描かず `portrait_error` になる(glm-4-6 が依頼文を丸写しした事例への対処) |
| `character_chat_models.py`  | `CharacterChatPlan` / `CharacterChatLookup` / `CharacterChatAppearanceOutput`（寛容 validator。上限は `consts/character_chat.py`。使ってよい調べ物の種類は検証 context の `allowed_kinds` で渡し、無ければ過去プレイの 5 種だけ。`web_search` の検索語は 8 語まで、現実世界の種類は 1 手番に種類ごと 1 件） |
