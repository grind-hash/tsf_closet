...
| `/api/character-chat` | `routes/character_chat_router.py` | キャラチャット（TSF シナリオを経由しないキャラクターとの会話）。`GET /threads`、`POST /threads/base`（拠点キャラ「セレナ」のスレッド。冪等）、`POST /threads`（過去セッション / 履歴 / Prompt Expander エントリの人物から作成。201）、`GET/DELETE /threads/{id}`、`POST /threads/{id}/messages/stream`（SSE: `status{phase: plan|reply|portrait|memory}` / `chat_chunk` / `chat_done` / `portrait_image` / `portrait_error` / `cost` / `complete` / `error`）、`PUT /threads/{id}/appearance`（姿の差し替え）、`POST /threads/{id}/appearance/reset`（作成時点の姿へ戻す。画像生成なし）、`POST /threads/{id}/portrait/stream`（立ち絵の描き直し。SSE）、`GET /images/{thread_id}/{filename}`。エラーは `detail.code`（`thread_not_found` / `source_not_found` / `image_not_found` は 404、他 400） |
...
| `character_chat_prompts.py` | 判定 LLM / 返答本文 / 着替えの外見タグ更新 / スレッド要約のプロンプト（純関数）。ユーザーメモリは `memory_block` の弱い枠で包む（`build_memory_priority_instruction` の「最優先」は使わない） |
