...
> **US1/US2/US3 の E2E テスト不適用について**: US1-US3 は LLM 生成テキストの語調・内容を変更するが、出力の非決定性により E2E での自動検証が困難なため Playwright テストの対象外とする。代わりにユニットテスト（Phase 9: T037）でプロンプト構築ロジックを検証し、最終的な出力品質は手動の定性レビューで確認する（原則 II への根拠）。
