...
async def _load_session_bundle(session_id: str) -> Optional[_SessionBundle]:
    ...
    for hist in history_rows:
        ...
        feeling_content = "" if skip_feeling else f"💭 {feeling_text}"

        hist_images: list[_ImageRef] = []
        main = _resolve_history_image_path(hist)
        if main is not None and hist.id not in used_history_ids:
            hist_images.append(_ImageRef(asset_id=f"hist_{hist.id}", abs_path=main))
...
async def build_novel_html_zip(session_id: str) -> tuple[bytes, str]:
    ...
    if not _NOVEL_HTML_TEMPLATE.exists() or not _NOVEL_CSS.exists():
