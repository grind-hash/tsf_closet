...
class User(Base):
    ...
    tts_engine_dir: Mapped[str | None] = mapped_column(Text, nullable=True)
    # 音声合成エンジンの待ち受けポート。NULL のときは AIVIS_ENGINE_BASE_URL の
    # ポートを使う。既定の 10101 が他用途で使用済みの場合などに変更する。
    tts_engine_port: Mapped[int | None] = mapped_column(Integer, nullable=True)
    tts_model_dir: Mapped[str | None] = mapped_column(Text, nullable=True)
    tts_speaker_id: Mapped[str | None] = mapped_column(String, nullable=True)
    tts_style_id: Mapped[str | None] = mapped_column(String, nullable=True)
    tts_output_format: Mapped[str] = mapped_column(
        String, default="wav", nullable=False, server_default="wav"
    )
    self_profile_json: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    prompt_expander_settings_json: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
...
class Session(Base):
    ...
    play_memory_system_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    play_memory_user_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    play_memory_system_updated_at: Mapped[datetime | None] = mapped_column(
        nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        default=func.current_timestamp(), nullable=False
...
class History(Base):
    ...
    before_description: Mapped[str | None] = mapped_column(Text, nullable=True)
    after_description: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    tag: Mapped[TransformationTag | None] = relationship(
...
class Conversation(Base):
    ...
    instruction_type: Mapped[str | None] = mapped_column(Text, nullable=True)
    attached_image_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    related_history_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        default=func.current_timestamp(), nullable=False
    )
...
class AdventureRun(Base):
    ...
    opening_state_json: Mapped[str | None] = mapped_column(Text, nullable=True)
    current_image_path: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    portrait_image_path: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class UserAchievement(Base):
    ...
    achieved_at: Mapped[str | None] = mapped_column(String, nullable=True)
...
class PromptExpanderEntry(Base):
    ...
    text_model: Mapped[str | None] = mapped_column(String, nullable=True)
    seed: Mapped[int | None] = mapped_column(Integer, nullable=True)
    i2i_strength: Mapped[float | None] = mapped_column(Float, nullable=True)
    i2i_noise: Mapped[float | None] = mapped_column(Float, nullable=True)
    image_size: Mapped[str | None] = mapped_column(String, nullable=True)
    # 漫画モード（V5 のコマ割り）で拡張したプロンプトか。panel_count は None がおまかせ
    manga_mode: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False, server_default="0"
    )
    manga_panel_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    reference_history_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("history.id", ondelete="SET NULL"), nullable=True
    )
    reference_entry_id: Mapped[str | None] = mapped_column(
        String,
        ForeignKey("prompt_expander_entries.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    reference_strength: Mapped[float | None] = mapped_column(Float, nullable=True)
    reference_fidelity: Mapped[float | None] = mapped_column(Float, nullable=True)
    # インペイント（部分修正）で生成したか。マスクは画像と同じ場所へ _mask.png で残す
    inpaint: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False, server_default="0"
    )
    inpaint_mask_path: Mapped[str | None] = mapped_column(Text, nullable=True)
