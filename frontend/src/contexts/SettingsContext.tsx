...
// 累計コストは設定 JSON とは別キーで保持する（旧バージョンとの互換）
const LEGACY_TOTAL_COST_KEY = "api_total_cost";
...
function loadInitialState(initial: SettingsState): SettingsState {
  try {
    ...
    removeStorage("local", "preserve_presets");
    const saved = readStorage("local", STORAGE_KEY);
    const legacyTotalCost = readStorage("local", LEGACY_TOTAL_COST_KEY);
    if (saved) {
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      writeStorage("local", STORAGE_KEY, JSON.stringify(rest));
      writeStorage("local", LEGACY_TOTAL_COST_KEY, String(state.totalCost));
    } catch (error) {
